...
* Loaded London MUDDI data into MariaDB.
* Updated the MariaDB OGC API - Records implementation to align with the current state of the Part 1 editor's draft.
* Updated the MariaDB OGC API - Processes implementation to align with the current state of the Part 1 & 2 editor's drafts.

===== Code sprint results

====== MUDDI

The goal of MariaDB's MUDDI work was to load MUDDI data into MariaDB and then serve the data out through OGC APIs. MUDDI data was loaded into MariaDB and could be accessed through OGC APIs at the following landing page:
...
* The first concerned "faceted" searching (aka Aggregations) which resulted in a https://github.com/opengeospatial/ogcapi-records/pull/320[pull request being created for the proposal]. The decision was made that this capability would not be part of Part 1 - Core but would be in a stand-alone part.
