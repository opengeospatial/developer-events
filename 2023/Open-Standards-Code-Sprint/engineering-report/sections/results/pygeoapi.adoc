...
A live demo was given which exemplified the capabilities of PubSub as a workflow for metadata catalog management, using the WMO
...
the catalog is subscribed to an MQTT broker which is publishing messages of new discovery metadata records. On notification of
new metadata, the catalog is triggered to validate and ingest the metadata into a metadata collection, thereby making it available
via OGC API - Records.
