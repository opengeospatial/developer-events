...
STAC is a multi-part specification that includes the following consituent parts:
...
*  https://github.com/radiantearth/stac-spec/blob/master/collection-spec/collection-spec.md[STAC Collection] extends the STAC Catalog to offer additional information such as the extents, keywords, license, providers, and other elements that describe STAC Items that grouped within the Collection.
